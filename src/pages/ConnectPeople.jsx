...
//       // Make the API call
//       const response = await axios.get("https://updatedbackendcc.onrender.com/api/users/requests", config)
...
//     try {
//       setLoading((prev) => ({ ...prev, suggestions: true }))
//       const response = await axios.get("https://updatedbackendcc.onrender.com/api/users/suggested", config)
...
//       setIsSearching(true)
//       setLoading((prev) => ({ ...prev, search: true }))
//       const response = await axios.get(`https://updatedbackendcc.onrender.com/api/users/search?query=${searchQuery}`, config)
...
//   const handleFollow = async (userId) => {
//     try {
//       const response = await axios.post(`https://updatedbackendcc.onrender.com/api/users/follow/${userId}`, {}, config)
...
//       const response = await axios.post(`https://updatedbackendcc.onrender.com/api/users/respond/${requestId}`, { action }, config)
...
const ConnectPeople = () => {
  ...
  const [connectionsModalKey, setConnectionsModalKey] = useState(0) // Force refresh modal
  ...
  const fetchSuggestedUsers = async () => {
    try {
      ...
      // Fetch current user's connections to check status
      try {
        const connectionsResponse = await axios.get(
          "https://updatedbackendcc.onrender.com/api/users/connections",
          config,
        )
        const sentRequestsResponse = await axios.get(
          "https://updatedbackendcc.onrender.com/api/users/sent-requests",
          config,
        )
        ...
        const updatedUsers = users.map((user) => {
          ...
          const requestSent = sentRequests.some((request) => request.following._id === user._id)
  ...
  const searchUsers = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://updatedbackendcc.onrender.com/api/users/search?query=${searchQuery}`,
        config,
      )
      ...
      // Check connection status for search results
      try {
        const connectionsResponse = await axios.get(
          "https://updatedbackendcc.onrender.com/api/users/connections",
          config,
        ...
        console.error("Error fetching connection status for search:", connectionError)
  ...
  const handleRequestResponse = async (requestId, action) => {
    try {
      ...
      const response = await axios.post(
        `https://updatedbackendcc.onrender.com/api/users/respond/${requestId}`,
        { action },
        config,
      )
      ...
      // If accepted, refresh suggested users list and force refresh connections modal
      if (action === "accept") {
        fetchSuggestedUsers()
        setConnectionsModalKey((prev) => prev + 1) // Force refresh modal
      }
    } catch (error) {
      console.error(`Error ${action}ing request:`, error)
      toast.error(error.response?.data?.message || `Failed to ${action} request. Please try again.`)
