        ...
        <div
          ...
          style={{
            backgroundImage: profile.coverPhotoUrl
              ? `url(${profile.coverPhotoUrl.startsWith("http") ? profile.coverPhotoUrl : `${API_BASE_URL}${profile.coverPhotoUrl}`})`
              : undefined,
            backgroundSize: "cover",
            backgroundPosition: "center",
          }}
                  ...
                  <div className="mt-4 md:mt-0 flex space-x-3">
                    {connectionStatus === "none" && (
                      <button
                        onClick={sendConnectionRequest}
                        disabled={loading.action}
                        className="px-6 py-3 bg-gradient-to-r from-indigo-600 to-blue-600 text-white rounded-lg hover:from-indigo-700 hover:to-blue-700 transition-all duration-200 flex items-center font-medium shadow-lg disabled:opacity-50"
                      >
                        {loading.action ? (
                          <span className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></span>
                        ) : (
                    ...
                    <button
                      onClick={startConversation}
                      disabled={loading.action}
                      className="px-6 py-3 border-2 border-indigo-300 text-indigo-700 rounded-lg hover:bg-indigo-50 transition-all duration-200 flex items-center font-medium disabled:opacity-50"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className="h-5 w-5 mr-2"
