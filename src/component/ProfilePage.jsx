...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const endpoint = userId
          ? `https://updatedbackendcc.onrender.com/api/profiles/${userId}`
          : "https://updatedbackendcc.onrender.com/api/profiles/me"
  ...
  const fetchPostCount = async (targetUserId) => {
    try {
      const response = await fetch(`https://updatedbackendcc.onrender.com/api/posts/user/${targetUserId}/count`)
      if (response.ok) {
